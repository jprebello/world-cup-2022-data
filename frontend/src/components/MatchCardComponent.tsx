...
function MatchCard({ match, alternateColor = false }: MatchCardProps) {
  const bg = alternateColor ? 'bg-primary text-white' : 'bg-white text-black'
  const dateColor = alternateColor ? 'text-white/50' : 'text-black/30'
  ...
  return (
    <div className={`relative flex items-center justify-between w-full h-16 rounded-full px-4 ${bg}`}>
